...
const AdminDashboard = () => {
    ...
    const handleModalConfirm = async (duration) => {
        ...
        try {
            if (pendingAction.type === 'PAYMENT') {
                ...
                showToast(`Payment Confirmed. Code: ${response.data.generated_code}`);
            } else if (pendingAction.type === 'CODE') {
                const response = await api.post('/admin/codes/generate/', { duration: parseInt(duration) });
                showToast(`Master Code Generated: ${response.data.code}`);
            }
            handleRefresh();
        } catch (error) {
    ...
    const handleRevokeLicense = (id) => {
        setActionModal({
            ...
            message: 'Are you sure you want to revoke this license? Please provide a reason for the log.',
    ...
    const handleRevokeCode = (id) => {
        setActionModal({
            ...
            message: 'This code will be immediately invalidated and cannot be used by any device. Proceed?',
            confirmText: 'Invalidate Code',
            onConfirm: async () => {
                setActionLoading(true);
                try {
                    await api.post(`/admin/codes/${id}/revoke/`);
                    showToast("Code invalidated");
                    handleRefresh();
                    setActionModal(prev => ({ ...prev, isOpen: false }));
                } catch (error) {
    ...
    const handleRevokeTrial = (fingerprint) => {
        setActionModal({
            isOpen: true,
            type: 'danger',
            title: 'Revoke Trial Period',
            message: 'The trial for this device will end immediately. The user will need a valid code to continue.',
            confirmText: 'End Trial',
            onConfirm: async () => {
                setActionLoading(true);
                try {
                    await api.post(`/admin/devices/${fingerprint}/revoke_trial/`);
                    showToast("Trial period revoked");
                    handleRefresh();
                    setActionModal(prev => ({ ...prev, isOpen: false }));
                } catch (error) {
    ...
    const handleUpdateExpiry = (id, currentExpiry) => {
        const defaultDate = currentExpiry ? currentExpiry.split('T')[0] : new Date().toISOString().split('T')[0];
        setActionModal({
            isOpen: true,
            type: 'info',
            title: 'Update Expiration',
            message: 'Enter the new target expiration date for this license.',
            confirmText: 'Update Date',
            requiresInput: true,
            inputPlaceholder: 'YYYY-MM-DD',
            onConfirm: async (newExpiry) => {
                setActionLoading(true);
                try {
                    await api.post(`/admin/licenses/${id}/update_expiry/`, { new_expiry: newExpiry });
                    showToast("Subscription date updated");
                    handleRefresh();
                    setActionModal(prev => ({ ...prev, isOpen: false }));
                } catch (error) {
                    showToast("Invalid date format or update failed", 'error');
                }
                setActionLoading(false);
            }
        });
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        className={clsx(
                            "fixed bottom-8 left-1/2 z-[200] px-6 py-4 rounded-2xl shadow-2xl border flex items-center gap-3 backdrop-blur-xl min-w-[300px]",
